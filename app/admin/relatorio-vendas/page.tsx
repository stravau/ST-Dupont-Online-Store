...
import { salesByStore, salesLog, dayWindow } from "@/lib/pos-reports";
...
export default async function DailyReportPage({ searchParams }: { searchParams: Promise<{ date?: string }> }) {
  ...
  const now = new Date();
  const ymd = date && /^\d{4}-\d{2}-\d{2}$/.test(date) ? date : ymdOf(now);
  ...
  const [stores, log] = await Promise.all([
    ...
    salesLog(boutiques, from, to, 1000),
  ]);
  ...
  const dayLabel = day.toLocaleDateString("pt-PT", { weekday: "long", day: "2-digit", month: "long", year: "numeric" });
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Escolhe um dia para ver o resumo e exportar o registo em Excel (mesmo formato e fórmulas do controlo ECI)."
      />

      <div className="mt-6">
        <ReportDatePicker date={ymd} />
      </div>

      <p className="mt-8 font-serif text-xl text-ink capitalize">{dayLabel}</p>
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        {multi && (
          <div className="border border-line border-l-[3px] border-l-ink bg-ink/[0.03] p-5">
            <p className="overline text-[0.55rem] text-gold">Total do dia · as duas lojas</p>
      ...
      {/* The day's register — a preview of what the Excel export contains. */}
      <section className="mt-8">
        <h2 className="font-serif text-lg text-ink">Registo do dia</h2>
        {log.length === 0 ? (
          <p className="mt-3 text-sm text-muted">Sem vendas registadas neste dia.</p>
        ) : (
