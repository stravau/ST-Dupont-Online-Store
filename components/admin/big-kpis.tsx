...
function DeltaBadge({ pct }: { pct: number | null }) {
  ...
  if (Math.abs(pct) < 0.5) {
    return (
      <span className="ml-2 inline-flex items-center gap-1 text-[0.68rem] tracking-wide text-muted">
        ≈ mesmo
      </span>
    );
  }
    ...
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <div className="painel-card hero-reveal p-5" style={{ animationDelay: "0ms" }}>
        ...
        <dl className="mt-4 grid grid-cols-2 gap-3 border-t border-line pt-3 text-[0.72rem] text-muted">
          ...
          <div>
            <dt className="text-[0.58rem] tracking-[0.1em] text-muted uppercase">Vendas · Devs</dt>
