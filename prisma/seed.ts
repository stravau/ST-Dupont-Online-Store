...
interface SplitPart {
  ...
  skus?: string[];     // exact SKU list (used by the generated leather splits,
                       // where colour is embedded mid-SKU so prefixes don't work)
}
const SPLIT_PRODUCT: Record<string, SplitPart[]> = {
  // Bundle splits — generated by scripts/gen-bundle-splits.ts from the
  // www catalogue (leather / writing / accessories; one product per
  // model). Spread first so the hand-written entries below can override.
  ...BUNDLE_SPLITS,
...
function transform(list: readonly SeedProduct[]): SeedProduct[] {
  ...
  for (const p of list) {
    ...
    // Bundle split — emit one product per SplitPart. A part matches its
    // variants either by exact SKU list (the generated leather splits,
    // where colour is embedded mid-SKU) or by SKU prefix (the hand-
    // written entries). Each variant name is rewritten to carry the part
    // label so cards read "Apex · Card Holder (225€) — Preto" instead of
    // a bare "Apex — Preto".
    const split = SPLIT_PRODUCT[newSlug] ?? SPLIT_PRODUCT[p.slug];
    if (split) {
      const assigned = new Set<string>();
      const partFor = (part: SplitPart) =>
        variants.filter((v) => {
      ...
      for (const part of split) {
        const partVariants = partFor(part);
      ...
      // Safety net — any variant the config didn't assign stays under the
      // original product so a colourway is NEVER silently lost (e.g. if
      // seed-data gains a SKU the generated split hasn't seen yet).
      const leftovers = variants.filter((v) => !assigned.has(v.sku));
      if (leftovers.length > 0) {
        console.warn(`[split] ${p.slug}: ${leftovers.length} unassigned variant(s) kept under original — ${leftovers.map((v) => v.sku).join(", ")}`);
        out.push({
          ...p,
          slug: newSlug,
          name: {
...
async function main() {
  ...
  // Resilience: create each product in its own try/catch. A single bad
  // row (e.g. a stray duplicate slug or EAN) must NEVER abort the loop
  // and leave a half-seeded catalogue — that's exactly what wiped the
  // base models before. Failures are logged and counted; the rest seed.
  let failed = 0;
  for (const p of finalProducts) {
    const categoryId = categoryIdBySlug.get(p.categorySlug);
    if (!categoryId) { console.error(`[seed] unknown category for ${p.slug} — skipped`); failed++; continue; }
    ...
    try {
      ...
      failed++;
      console.error(`[seed] FAILED ${p.slug}: ${(e as Error).message.slice(0, 140)}`);
    }
  }
  if (failed > 0) console.error(`[seed] ${failed} product(s) failed — see errors above.`);
